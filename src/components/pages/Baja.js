...
export default function Baja() {
  ...
  function checkFormValido(){
    setFormValido(
      formBaja.nombre && !formBaja.validaciones.nombre &&
      formBaja.cuit && !formBaja.validaciones.cuit &&
      formBaja.telefono && !formBaja.validaciones.telefono &&
      formBaja.email && !formBaja.validaciones.email &&
      formBaja.mensaje && !formBaja.validaciones.mensaje
    )
  }
  ...
  function handleTelefonoChange(event){
    var mensaje = null
    if (!event.target.value)
      mensaje = 'Teléfono/Celular obligatorio'
    else {
  ...
  function handleNombreChange(event){
    var mensaje = null
    if (!event.target.value)
      mensaje = 'Nombre/Apellido es obligatorio'
    ...
    <div className='baja'>
      <div className='baja-first-section'>
        <div className='baja-left-box'>
          ...
          <p className='baja-text'>Podrás realizar la baja de nuestros servicios o la devolución de tu préstamo personal dentro de los 10 días hábiles de solicitado, para realizarlo completá el siguiente formulario</p>
        </div>
      ...
      <div>
        <p className='baja-nota'>El/la USUARIO/A podrá revocar la aceptación del PRÉSTAMO dentro de los diez (10) días hábiles contados a partir de la fecha de perfeccionado el mismo, notificando de manera fehaciente o por el mismo medio en que fue contratado. El/la USUARIO/A deberá reintegrar el monto del PRESTAMO contratado dentro de las 24 hs. de solicitada, aclarándose que la revocación es sin costo ni responsabilidad alguna para el/la USUARIO/A, en la medida que no haya hecho uso del respectivo producto o servicio, y que, en el caso que lo haya utilizado, sólo se le cobrarán las comisiones y cargos previstos, proporcionados al tiempo de utilización del servicio o producto.</p>
        <p className='baja-nota-2'>Tambien podés escribirnos por correo a {window.baja.mail} o comunícate por teléfono al 0810-220-0570 o WhatsApp al {window.baja.telefono}</p>
      </div>
    </div>
  );
}
