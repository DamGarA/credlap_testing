...
export default function PagarFactura() {
  ...
  function handleCondicionesServicioChecked(event) {
    ...
    if (!condicionesServicioLeidas) {
      console.warn(
        "⚠️  [PagarFactura] Usuario intentó marcar checkbox sin leer condiciones"
      );
      mostrarAlerta(
  ...
  function onFormSubmit(event) {
    ...
    if (provinciasExcluidas.includes(formSolicitud.provincia))
      ...
      try {
        ...
        console.warn(
          "⚠️  [PagarFactura] No se pudo guardar datos en sessionStorage:",
          error
        );
      }
  ...
  function handleAdobeSignComplete() {
    ...
    try {
      ...
      console.warn(
        "⚠️  [PagarFactura] Error guardando estado de firma:",
        error
      );
    }
